    ...
    public static void MapBoletasEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/", async (CreateBoletaRequest request, ApplicationDbContext db) =>
        {
            var boleta = new Boleta
            {
                ...
                CriadoEm = request.CriadoEm ?? DateTime.Now
            };
        ...
        group.MapPut("/{id:int}", async (int id, UpdateBoletaRequest request, ApplicationDbContext db) =>
        {
            ...
            // Cascatear alteracoes para boletas ligadas (exceto ativoId, ticker e quantidade)
            var boletasLigadas = await db.Boletas
                .Where(b => b.BoletaPrincipalId == id)
                .ToListAsync();

            foreach (var ligada in boletasLigadas)
            {
                ligada.ContraparteId = request.ContraparteId;
                ligada.FundoId = request.FundoId;
                ligada.TipoOperacao = request.TipoOperacao;
                ligada.Volume = request.Volume;
                ligada.TipoPrecificacao = Enum.Parse<TipoPrecificacao>(request.TipoPrecificacao);
                ligada.NtnbReferencia = request.NtnbReferencia;
                ligada.SpreadValor = request.SpreadValor;
                ligada.DataFixing = request.DataFixing;
                ligada.TaxaNominal = request.TaxaNominal;
                ligada.PU = request.PU;
                ligada.Alocacao = request.Alocacao;
                ligada.Usuario = request.Usuario;
                ligada.Observacao = request.Observacao;
                ligada.DataLiquidacao = request.DataLiquidacao;
                ligada.Status = Enum.Parse<StatusBoleta>(request.Status);
            }
